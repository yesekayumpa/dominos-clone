...
import { Bell, Search, Sliders, MapPin, Plus } from 'lucide-react-native';
...
const HomeScreen = ({ navigation }) => {
  ...
  const popularPizzas = [
    { id: 'p1', name: 'Pepperoni Pizza', desc: 'Classic pepperoni & cheese', price: 6500, image: 'https://images.unsplash.com/photo-1628840042765-356cda07504e?w=200' },
    { id: 'p2', name: 'Margherita', desc: 'Fresh tomatoes & basil', price: 5000, image: 'https://images.unsplash.com/photo-1574071318508-1cdbab80d002?w=200' },
    { id: 'p3', name: 'BBQ Chicken', desc: 'Grilled chicken with BBQ', price: 7500, image: 'https://images.unsplash.com/photo-1565299624946-b28f40a0ae38?w=200' },
  ];
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <View style={styles.locationContainer}>
             ...
             <View style={styles.locationTextContainer}>
                <Text style={styles.locationLabel}>{t('common.hello')}, {user.name} 👋</Text>
        ...
        <View style={styles.heroContainer}>
           ...
           >
              <Text style={styles.heroText}>{t('menu.deliciousPizza')}</Text>
           </ImageBackground>
        ...
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>{t('menu.categories')}</Text>
          <TouchableOpacity>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.popularContainer}>
           {popularPizzas.map((pizza) => (
             <View key={pizza.id} style={styles.popularCard}>
                <Image source={{ uri: pizza.image }} style={styles.popularImage} />
                <View style={styles.popularInfo}>
                  <Text style={styles.popularName} numberOfLines={1}>{pizza.name}</Text>
                  <Text style={styles.popularDesc} numberOfLines={1}>{pizza.desc}</Text>
                  <View style={styles.popularFooter}>
                    <Text style={styles.popularPrice}>{pizza.price} FCFA</Text>
                    <TouchableOpacity style={styles.addBtn} onPress={() => addToCart(pizza)}>
        ...
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>{t('menu.bookRestaurants')}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  popularContainer: { flexDirection: 'row', marginBottom: 25 },
  popularCard: { width: 160, marginRight: 15, backgroundColor: Colors.white, borderRadius: 20, padding: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.1, shadowRadius: 10, elevation: 4 },
  popularImage: { width: '100%', height: 120, borderRadius: 15, backgroundColor: Colors.secondary, marginBottom: 10 },
  popularInfo: { flex: 1 },
  popularName: { fontSize: 16, fontWeight: 'bold', color: Colors.text, marginBottom: 4 },
  popularDesc: { fontSize: 12, color: Colors.textLight, marginBottom: 10 },
  popularFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  popularPrice: { fontSize: 14, fontWeight: 'bold', color: Colors.primary },
